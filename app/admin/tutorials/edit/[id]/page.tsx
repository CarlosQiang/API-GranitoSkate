...
export default function EditTutorialPage({ params }) {
  ...
  useEffect(() => {
    const fetchTutorial = async () => {
      try {
        ...
        console.log("Tutorial cargado:", data)
        ...
        // Formatear el JSON para mostrarlo en el textarea
        setPasosText(JSON.stringify(data.pasos || [], null, 2))
      } catch (err) {
        console.error("Error al cargar tutorial:", err)
        setError(err.message || "Error al cargar los datos del tutorial")
      } finally {
        setLoading(false)
      }
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!response.ok) {
        const data = await response.json()
        throw new Error(data.error || "Error al actualizar el tutorial")
      }

      router.push("/admin/tutorials")
    } catch (error) {
      console.error("Error:", error)
      setError(error.message || "Error al actualizar el tutorial")
    } finally {
      setSaving(false)
    }
  }
